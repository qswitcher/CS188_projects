...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        """
        values = self.qValues.get(state, None)
        return values[action] if values else 0.0
    ...
    def computeValueFromQValues(self, state):
        ...
        return self.getQValue(state, action) if action else 0.0
    ...
    def update(self, state, action, nextState, reward):
        ...
        # get Qsa
        temp = self.qValues.get(state, util.Counter())
        self.qValues[state] = temp
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for (key, fi) in self.featExtractor.getFeatures(state, action).items():
            self.weights[key] += self.alpha * difference * fi
